...
def Main(argv):
    ...
    argparser = argparse.ArgumentParser(
        description="Prints a list of dependencies (projects from which files are #included) for the specified project, or a list of known projects."
    )
    ...
    config.argparser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Only print the direct project dependencies. Otherwise both direct and indirect dependencies are printed.",
    )
    config.argparser.add_argument(
        "-p",
        "--project-name",
        ...
        help="The name of the project for which we will generate the dependency list. The '?', '^' and '~' can be used to: list all known projects, list projects that are not depended on by any other project (top level projects) and list all projects that depend on no other project respectively.",
    )
    config.argparser.add_argument(
        "--dot",
        dest="printDot",
        action="store_true",
        default=False,
        help="Instead of printing the dependencies print a dot graph instead.",
    )
    config.argparser.add_argument(
        "--example-dot-config",
        dest="exampleDot",
        action="store_true",
        default=False,
        help="Prints an example dot configuration on stdout. Only projects that would have been printed without this option will be included in the configuration.",
    )
    config.argparser.add_argument(
        "--dot-config",
        dest="dotConfig",
        metavar="<dot-config-file>",
        help="The contents of this file will be added to the generated graph output before the edges are specified.",
    )
    ...
    if config.printExampleConfig:
        ...
        if database is not None:
                        ...
                        if config.directOnly:
                            try:
                                ...
                                config.messagePrinter.error(
                                    "Couldn't find {0} dependencies. Are you sure the project exists?".format(project)
                                )
                                return False
                        else:
            ...
            config.messagePrinter.info(
                "Error! No database or config file provided. Couldn't read or generate database."
            )
