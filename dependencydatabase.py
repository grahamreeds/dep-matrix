...
class DependencyScriptConfiguration(object):
    ...
    def __init__(self, argv=None, argparser=None):
        ...
        if argparser is None:
            self.argparser = argparse.ArgumentParser(
                description="Generates an SQLite3 database containing all of the #include dependencies in your project."
            )
        else:
        ...
        self.argparser.add_argument(
            ...
            default=False,
            dest="debugMessages",
            help="Print the debug messages. Very verbose, use only in development.",
        )
        self.argparser.add_argument(
            "--verbose",
            action="store_true",
            default=False,
            dest="verbose",
            help="Print the info messages about progress and status.",
        )
        self.argparser.add_argument(
            "--silence-errors", action="store_true", default=False, dest="silenceErrors", help="Don't print any errors."
        )
        self.argparser.add_argument(
            "-c",
            "--config-filename",
            ...
            help="Specifies the path to the configuration file to use.",
        )
        self.argparser.add_argument(
            "-f",
            "--database-filename",
            ...
            metavar="<db-filename>",
            help="Specifies the filename of the database. Note: specifying this option overrides the filename in the configuration file.",
        )
        self.argparser.add_argument(
            "-s",
            "--source-path",
            dest="sourcePath",
            metavar="<source-path>",
            help="Specifies the path to the root of the source code.",
        )
    ...
    def Configure(self, argv):
        ...
        if os.path.exists(self.scriptIni):
            ...
            msg = "No configuration file found. Searched, current directory and script directory directory for {0}.".format(
                self.scriptIni
            )
    ...
    def Configure(self, config):
        ...
        for p in self.jsonObjectStrings:
            try:
                ...
                config.messagePrinter.error(
                    "The [{0}] option in the [{1}] section has an invalid JSON object!".format(p, sectionName)
                )
        ...
        for group in self.jsonObjects["ProjectGroupsList"]:
            if config.sourcePath is not None:
                if group.pathPrefix is not None:
                    groupPathPrefix = group.pathPrefix = toPosixPath(
                        os.path.normpath(os.path.join(config.sourcePath, group.pathPrefix))
                    )
                else:
            ...
            for project in group.projects:
                ...
                if groupPathPrefix is not None:
                    ...
                    if project.includePath is not None and len(project.includePath) > 0:
                        project.includePath = toPosixPath(
                            os.path.normpath(os.path.join(groupPathPrefix, project.includePath))
                        )

                if group.pathPrefix is not None and not os.path.isdir(group.pathPrefix):
                    config.messagePrinter.error(
                        'The group "{0}" path-prefix "{1}" does not exist.'.format(group.name, group.pathPrefix)
                    )
                if not os.path.isdir(project.path):
                    config.messagePrinter.error(
                        'The project "{0}" path "{1}" does not exist.'.format(project.name, project.path)
                    )
                if project.includePath is not None and not os.path.isdir(project.includePath):
                    config.messagePrinter.error(
                        'The project "{0}" include-path "{1}" does not exist.'.format(project.name, project.includePath)
                    )
...
"""
    ...
    def AddProject(self, projectName, solutionPath, hierarchyLevel):
        ...
        try:
            self.cur.execute(
                "INSERT INTO Project (SolutionPath, Name, HierarchyLevel) VALUES (?, ?, ?);",
                (solutionPath, projectName, hierarchyLevel),
            )
        except:
            self.cur.execute(
                "UPDATE CodeFile SET Name = ?, HierarchyLevel = ? WHERE SolutionPath = ?;",
                (projectName, hierarchyLevel, solutionPath),
            )
    ...
    def AddFile(self, filename, project, solutionPath, exists):
        ...
        try:
            self.cur.execute(
                "INSERT INTO CodeFile (SolutionPath, Project, Filename) VALUES (?, ?, ?);",
                (solutionPath, project, filename),
            )
        except:
            self.cur.execute(
                "UPDATE CodeFile SET Project = ?, Filename = ? WHERE SolutionPath = ?;",
                (project, filename, solutionPath),
            )
    ...
    def AddInclude(
        ...
        if normIncludeText != includeText:
            if self.errorLogger:
                self.errorLogger.write(
                    "Error, #include directive with non-posix path!"
                    + includeText
                    + " included in "
                    + solutionPath
                    + "\n"
        ...
        try:
            self.cur.execute(
                "INSERT INTO IncludeDirective (CodeFileSolutionPath, IncludeText, IncludeType, IncludeFilename, IncludeProject, IncludeSolutionPath, LineNumber) VALUES (?, ?, ?, ?, ?, ?, ?);",
                (
                    solutionPath,
                    includeText,
                    includeType,
            ...
            if self.messagePrinter:
                msg = (
                    "Unknown exception for: INSERT INTO IncludeDirective (CodeFileSolutionPath, IncludeText, IncludeType, IncludeFilename, IncludeProject, IncludeSolutionPath) VALUES ("
                    + repr(solutionPath)
                    + ","
                    + repr(includeText)
                    + ","
                    + repr(includeType)
                    + ","
                    + repr(includeFilename)
                    + ","
                    + repr(includeProject)
                    + ","
                    + repr(includeSolutionPath)
                    + ","
                    + repr(lineNumber)
                    + ");"
                )
...
class SolutionProcessor(object):
    ...
    def GetIncludeFileAbsolutePath(self, absoluteFilepath, includetext, isLocalInclude=True):
        ...
        if os.path.isfile(localFilePath):
            ...
            if len(potentials) == 1:
                ...
                self.config.messagePrinter.error(
                    'include text "{0}" in {1} matches multiple files:'.format(includetext, absoluteFilepath)
                )
    ...
    def AddIncludeTupleToDatabase(self, filepath, solPath, includeTuple, isLocalInclude=True):
        ...
        fileList = self.database.AddInclude(
            solPath, include, includeType, includeFilename, incProject, solIncPath, lineNumber
        )
    ...
    def PopulateDatabase(self):
        ...
        for root, dirs, files in os.walk(self.solutionInfo.GetSolutionPath(), topdown=True):
            for name in files:
                ...
                if self.fileFilter and (
                    self.fileFilter.IsIncluded(filepath) and not self.fileFilter.IsExcluded(filepath)
                ):
                    ...
                    if processedCounter % 1000 == 0:
                        ...
                        self.config.messagePrinter.info(
                            "Processed {0: >4}, skipped {1: >6} files".format(processedCounter, skippedCounter)
                        )
                else:
